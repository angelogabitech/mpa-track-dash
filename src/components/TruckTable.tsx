...
import { CheckCircle2, XCircle, TrendingUp, Trash2, ShieldCheck, ShieldX, Search } from 'lucide-react';
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {trucks.map((truck, i) => {
              ...
              const Icon = cfg.icon;
              const isRejected = truck.status === 'rejected';
              const hasResults = truck.achievedMPa > 0;
              ...
              return (
                ...
                >
                  ...
                  <td className={cn('p-3 text-right font-medium', isRejected ? 'text-status-rejected' : 'text-card-foreground')}>
                    {hasResults ? (truck.achievedMPa).toFixed(1) : <span className="text-muted-foreground">—</span>}
                  </td>
