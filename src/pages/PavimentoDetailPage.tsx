...
export default function PavimentoDetailPage() {
  ...
  const handleAddTruck = async () => {
    if (!invoice || !arrDate || !arrTime || !departureFromPlant || !unloadStart || !unloadEnd || !departureFromSite || !volume || !expectedMPa || !supplier || !slump) return;
    setSavingTruck(true);
  ...
  const truckFormValid = invoice && arrDate && arrTime && departureFromPlant && unloadStart && unloadEnd && departureFromSite && volume && expectedMPa && supplier && slump;
      ...
      {pavimento.trucks.length === 0 ? (
        ...
        <>
          <TruckTable
            ...
            onRemove={(truckId) => removeTruck(pavimento.id, truckId)}
                      ...
                      {(truck.specimens || []).map(sp => {
                        ...
                        return (
                          <div key={sp.id} className={cn(
                            ...
                            <button onClick={() => removeSpecimen(pavimento.id, truck.id, sp.id)}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {/* Time control */}
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Controle de Tempo</p>
