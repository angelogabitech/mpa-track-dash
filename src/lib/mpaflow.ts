...
export function calculateMetrics(pavimentos: Pavimento[]): Metrics {
  ...
  const evaluatedTrucks = allTrucks.filter(t => t.mpa28d !== undefined);
  const evaluatedCount = evaluatedTrucks.length;
  const pendingCount = totalTrucks - evaluatedCount;
  const approvedCount = evaluatedTrucks.filter(t => t.status === 'approved').length;
  const rejectedCount = evaluatedTrucks.filter(t => t.status === 'rejected').length;
  const aboveCount = evaluatedTrucks.filter(t => t.status === 'above').length;
  const complianceRate = evaluatedCount ? ((approvedCount + aboveCount) / evaluatedCount) * 100 : 0;

  const trucksWithResults = evaluatedTrucks;
  ...
  const rejectedTrucks = evaluatedTrucks.filter(t => t.status === 'rejected');
  ...
  return {
    totalVolume, totalTrucks, evaluatedCount, pendingCount, approvedCount, rejectedCount, aboveCount,
...
export function getSupplierMetrics(pavimentos: Pavimento[]): SupplierMetrics[] {
  ...
  return Array.from(supplierMap.entries()).map(([name, trucks]) => {
    ...
    const evaluatedTrucks = trucks.filter(t => t.mpa28d !== undefined);
    const conforming = evaluatedTrucks.filter(t => t.status !== 'rejected').length;
    const complianceRate = evaluatedTrucks.length ? (conforming / evaluatedTrucks.length) * 100 : 0;
    const trucksWithResults = evaluatedTrucks;
    ...
    const rejectionRate = evaluatedTrucks.length ? ((evaluatedTrucks.length - conforming) / evaluatedTrucks.length) * 100 : 0;
...
export function getPavimentoRanking(pavimentos: Pavimento[]) {
  return pavimentos.map(p => {
    const evaluated = p.trucks.filter(t => t.mpa28d !== undefined);
    const total = evaluated.length;
    const conforming = evaluated.filter(t => t.status !== 'rejected').length;
...
export function getMPaEvolutionData(trucks: Truck[]) {
  return trucks.map((t, i) => ({
    ...
    achieved: t.mpa28d,
    predicted: t.mpa28d === undefined ? t.predictedMpa28d : undefined,
    mpa7d: t.mpa7d,
    mpa28d: t.mpa28d,
  }));
}
...
export function getStatusDistribution(trucks: Truck[]) {
  const evaluated = trucks.filter(t => t.mpa28d !== undefined);
  const approved = evaluated.filter(t => t.status === 'approved').length;
  const rejected = evaluated.filter(t => t.status === 'rejected').length;
  const above = evaluated.filter(t => t.status === 'above').length;
  return [
